...
*   **API キー管理:**
    *   サーバー起動時に環境変数 `GEMINI_API_KEY` を読み込み
    *   フロントエンドからキーを入力する必要はありません
*   **データ管理:**
...
バックエンドは起動時に環境変数 `GEMINI_API_KEY` を読み込みます。Vercel ではプロジェクト設定の Environment Variables にキーを登録してください。ローカル開発では `.env` ファイルに `GEMINI_API_KEY=...` を記述しておくことで自動的に読み込まれます。フロントエンドでのキー入力は不要です。

### 5. 使用方法

1.  事前に `GEMINI_API_KEY` を設定しておきます。
2.  **音声入力:**
