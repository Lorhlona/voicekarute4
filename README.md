...
バックエンドは起動時に環境変数 `GEMINI_API_KEY` を読み込みます。Vercel でデプロイする場合は、プロジェクトの **Environment Variables** にこの変数を設定してください。ローカルで開発する場合は `.env` ファイルに `GEMINI_API_KEY=YOUR_KEY` を記述します。
