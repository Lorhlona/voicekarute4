...
*   **API キー管理:**
    *   環境変数 `GEMINI_API_KEY` からキーを読み込み、起動時に自動で設定
*   **データ管理:**
...
バックエンドは起動時に環境変数 `GEMINI_API_KEY` を読み込みます。Vercel でデプロイする場合はプロジェクト設定の **Environment Variables** に `GEMINI_API_KEY` を追加してください。ローカル開発ではルートに `.env` ファイルを作成し、`GEMINI_API_KEY=取得したキー` を記述します。
