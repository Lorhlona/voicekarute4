...
function App() {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(false); // Password gate
  ...
  // No API key input handling (key is provided via environment variable)
  ...
  const handleClearData = async () => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            setStatus(isKeySet ? '待機中' : 'APIキー未設定'); // Reset status
        } else {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      setSelectedFile(null);
       setStatus(isKeySet ? '待機中' : 'APIキー未設定');
    }
  };
  ...
  if (!isAuthenticated) {
    return (
      <div className="container">
        ...
        />
        <button onClick={() => setIsAuthenticated(password === 'Puppy')}>ログイン</button>
      </div>
    );
  }
    ...
    <div className="container">
       ...
       {!isKeySet && keyStatusChecked && (
         <p style={{ color: 'red', textAlign: 'center' }}>
           サーバー側で API キーが設定されていません。環境変数 GEMINI_API_KEY を確認してください。
         </p>
       )}

       {/* Show Clear Data button if key is set */}
       {isKeySet && keyStatusChecked && (
      ...
      {/* Main App Sections (only enabled if authenticated and key is set) */}
      {isAuthenticated && isKeySet && keyStatusChecked && (
        <>
